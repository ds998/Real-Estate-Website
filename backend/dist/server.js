...
const agency_1 = __importDefault(require("./model/agency"));
const microlocation_1 = __importDefault(require("./model/microlocation"));
const city_1 = __importDefault(require("./model/city"));
const municipality_1 = __importDefault(require("./model/municipality"));
const real_estate_1 = __importDefault(require("./model/real_estate"));
...
router.route('/uploadmulphotos').post((request, response) => {
    try {
        uploadMulPhotos(request, response, (err) => {
            if (err) {
                console.log(err);
                response.status(410).json({ 'status': 'no' });
            }
...
router.route('/register').post((req, res) => {
    ...
    user_1.default.findOne({ 'username': username }, (err, user2) => {
        if (err)
            console.log(err);
        else if (user2)
            res.json({ 'user': 'exists' });
        else {
            user_1.default.findOne({ 'email': email }, (err2, user3) => {
                if (err2)
                    console.log(err2);
                else if (user3)
                    res.json({ 'user': 'email' });
                    ...
                    if (req.body.agency != "") {
                        user_1.default.findOne({ 'agency': req.body.agency, 'licence': req.body.licence }, (err3, user4) => {
                            if (err3)
                                console.log(err3);
                            else if (user4)
...
router.route('/remove_user').post((req, resp) => {
    let username = req.body.username;
    user_1.default.deleteOne({ 'username': username }).then(res => {
        resp.json(res);
    });
});
router.route('/undenied_users').get((req, resp) => {
    user_1.default.find({ 'approved': { $gt: -1 } }, (err, users) => {
        if (err)
            console.log(err);
        else
            resp.json(users);
...
router.route('/new_agency').post((req, resp) => {
    ...
    agency_1.default.findOne({ 'PIB': PIB }, (err, agent) => {
        if (err)
            console.log(err);
        else if (agent)
            resp.json({ 'agency': 'exists' });
        else {
            let a = new agency_1.default(req.body);
            a.save().then(a => {
                resp.json({ 'agency': 'ok' });
            }).catch(err2 => {
                resp.json({ 'agency': 'no' });
...
router.route('/city_municipalities').post((req, resp) => {
    municipality_1.default.find({ 'city': req.body.city }, (err, mps) => {
        if (err)
            console.log(err);
        else
            resp.json(mps);
...
router.route('/mp_microloc').post((req, resp) => {
    ...
    microlocation_1.default.find({ 'city': city, 'municipality': municipality }, (err, microlocations) => {
        if (err)
            console.log(err);
        else
            resp.json(microlocations);
    });
});
router.route('/new_microlocation').post((req, resp) => {
    let m = new microlocation_1.default(req.body);
    m.save().then(a => {
...
router.route('/all_microlocations').get((req, resp) => {
    microlocation_1.default.find((err, microlocations) => {
        if (err)
            console.log(err);
        else
            resp.json(microlocations);
...
router.route('/empty_microlocation').post((req, resp) => {
    real_estate_1.default.find({ 'microlocation': new mongoose_1.default.Types.ObjectId(req.body.microlocation) }, (err, r_est) => {
        if (err)
            console.log(err);
        else {
            resp.json(r_est);
...
router.route('/delete_microlocation').post((req, resp) => {
    microlocation_1.default.findByIdAndDelete(new mongoose_1.default.Types.ObjectId(req.body._id), (err, succ) => {
        if (err)
            console.log(err);
        else
            resp.json(succ);
...
router.route('/get_microlocation').post((req, resp) => {
    microlocation_1.default.findById(new mongoose_1.default.Types.ObjectId(req.body._id), (err, ml) => {
        if (err)
            console.log(err);
        else
            resp.json(ml);
...
router.route('/update_user').post((req, resp) => {
    ...
    user_1.default.findOne({ 'username': username }, (err, our_user) => {
        if (err)
            console.log(err);
        else if (our_user) {
            if (diff_email) {
                user_1.default.findOne({ 'email': req.body.email }, (err10, user6) => {
                    if (err10) {
                        console.log(err10);
                        return;
                    }
                        ...
                        if (diff_agency) {
                            user_1.default.findOne({ 'agency': req.body.agency, 'licence': req.body.licence }, (err11, user7) => {
                                if (err11) {
                                    console.log(err11);
                                    return;
                                }
                ...
                if (diff_agency) {
                    user_1.default.findOne({ 'agency': req.body.agency, 'licence': req.body.licence }, (err13, user13) => {
                        if (err13) {
                            console.log(err13);
                            return;
                        }
            ...
            return resp.json({ 'user': 'no' });
    });
});
router.route('/new_real_estate').post((req, resp) => {
    let re = new real_estate_1.default(req.body);
    re.save().then(re => {
...
router.route('/real_estates_by_user').post((req, resp) => {
    let username = req.body.username;
    real_estate_1.default.find({ 'advertiser.username': username }, (err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
    });
});
router.route('/sell_real_estate').post((req, resp) => {
    let _id = req.body._id;
    let n = req.body.n;
    real_estate_1.default.updateOne({ '_id': _id }, { 'selling_month': n }, (err, succ) => {
        if (err)
            console.log(err);
        else
            resp.json(succ);
...
router.route('/find_real_estate').post((req, resp) => {
    let _id = req.body._id;
    real_estate_1.default.findOne({ '_id': new mongoose_1.default.Types.ObjectId(_id) }, (err, real_estate) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estate);
    });
});
router.route('/update_real_estate').post((req, resp) => {
    let _id = req.body._id;
    real_estate_1.default.updateOne({ '_id': _id }, {
        name: req.body.name,
                        ...
                        if (diff_agency) {
                            user_1.default.findOne({ 'agency': req.body.agency, 'licence': req.body.licence }, (err11, user7) => {
                                if (err11) {
                                    ...
                                    user_1.default.updateOne({ 'username': username }, x, (err12, result5) => {
                                        if (err12) {
                                            console.log(err12);
                                            return;
                                        }
                            ...
                            user_1.default.updateOne({ 'username': username }, x, (err13, result6) => {
                                if (err13) {
                                    console.log(err13);
                                    return;
                                }
                ...
                if (diff_agency) {
                    user_1.default.findOne({ 'agency': req.body.agency, 'licence': req.body.licence }, (err13, user13) => {
                        if (err13) {
                            ...
                            user_1.default.updateOne({ 'username': username }, x, (err14, result7) => {
                                if (err14) {
                                    console.log(err14);
                                    return;
                                }
                    ...
                    user_1.default.updateOne({ 'username': username }, x, (err15, result8) => {
                        if (err15) {
                            console.log(err15);
                            return;
                        }
...
router.route('/agency_real_estates').post((req, resp) => {
    let PIB = req.body.PIB;
    real_estate_1.default.find({ 'advertiser.PIB': PIB }, (err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
...
router.route('/search').post((req, resp) => {
    ...
    real_estate_1.default.find(search_array, (err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
...
router.route('/type_average').post((req, resp) => {
    ...
    real_estate_1.default.find({ 'type': type, 'microlocation': new mongoose_1.default.Types.ObjectId(req.body.microlocation) }, (err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
...
router.route('/find_agency').post((req, resp) => {
    let PIB = req.body.PIB;
    agency_1.default.findOne({ 'PIB': PIB }, (err, agency_) => {
        if (err)
            console.log(err);
        else
            resp.json(agency_);
...
router.route('/average').post((req, resp) => {
    let microlocation = req.body.microlocation;
    real_estate_1.default.find({ 'microlocation': new mongoose_1.default.Types.ObjectId(req.body.microlocation) }, (err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
...
router.route('/get_last_five').get((req, resp) => {
    real_estate_1.default.find({}).sort({ _id: -1 }).limit(5).exec((err, real_estates) => {
        if (err)
            console.log(err);
        else
            resp.json(real_estates);
...
router.route('/insert_favorite').post((req, resp) => {
    let username = req.body.username;
    let _id = req.body._id;
    user_1.default.updateOne({ 'username': username }, { $push: { 'favorites': _id } }, (err, res) => {
        if (err)
            console.log(err);
        else
            resp.json(res);
...
router.route('/remove_favorite').post((req, resp) => {
    let username = req.body.username;
    let _id = req.body._id;
    user_1.default.updateOne({ 'username': username }, { $pull: { 'favorites': _id } }, (err, res) => {
        if (err)
            console.log(err);
        else
            resp.json(res);
...
router.route('/advanced_search').post((req, resp) => {
    ...
    if (max_construction_year > 0)
        construction_year.$lte = max_construction_year;
    ...
    let monthly_utilities = {};
    monthly_utilities.$gte = min_monthly_utilities;
    if (max_monthly_utilities > 0)
        monthly_utilities.$lte = max_monthly_utilities;
